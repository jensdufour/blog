...
**Automated Ubuntu Entra ID authentication** represents the next evolution in enterprise Linux deployment. Building on the manual configuration methods detailed in our previous articles on [Ubuntu Entra ID Authentication](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-entra-id-authd) and [Ubuntu Intune Enrollment](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-intune-enrollment), this guide introduces a complete automation solution that eliminates manual setup while enforcing security from the moment of installation.

![Web-based configurator generating Ubuntu autoinstall files](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/ubuntu-entra-automation-configurator.png) *Figure 1: Web-based configurator for automated Ubuntu Entra ID deployment*
...
> **Prerequisites:** Familiarity with the concepts in our [Ubuntu Entra ID Authentication](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-entra-id-authd) article is recommended but not required.
...
In our [previous article on Ubuntu Entra ID Authentication](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-entra-id-authd), we explored the manual configuration process:
...
![Comparison diagram: Manual vs Automated deployment timeline](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/manual-vs-automated-timeline.png) *Figure 2: Manual configuration takes 30+ minutes post-install; automation completes during installation*
...
Before generating your automated installation media, you need an **Azure App Registration** configured for Ubuntu device authentication. This is identical to the setup in our [manual authentication guide](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-entra-id-authd), but we’ll cover it here for completeness.

### Why App Registration is Required

Ubuntu devices using AuthD authenticate via **OAuth 2.0 device code flow**. Azure needs to know:

* Which applications are allowed to request authentication
* What permissions they have
* Where to send authentication responses (redirect URI)

Without an app registration, Azure will reject login attempts from your Ubuntu devices.
...
![Azure App Registration creation screen](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/azure-app-registration-create.png) *Figure 3: Creating a new App Registration for Ubuntu authentication*

**Configuration:**

* **Name:** `Ubuntu-Device-Auth` (or your preferred name)
* **Supported account types:** Single tenant (recommended)
* **Redirect URI:**
  + Platform: **Public client/native (mobile & desktop)**
  + URI: `msauth://com.microsoft.identity.client.sample.local/SignatureHash`

> **Important:** This specific redirect URI is required by the AuthD broker. It’s the default used by Microsoft’s authentication libraries for public client applications.
...
![Azure App Registration authentication settings](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/azure-app-authentication-settings.png) *Figure 4: Enabling public client flows for device code authentication*
...
![Azure App Registration API permissions](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/azure-app-api-permissions.png) *Figure 5: Required Microsoft Graph delegated permissions*

4. Click **Grant admin consent for [Your Tenant]**
5. Confirm the consent grant

### Step 4: Retrieve Your IDs

From the **Overview** page, copy these values (you’ll need them for the configurator):

| Field | Description | Example |
| --- | --- | --- |
| **Directory (tenant) ID** | Your Entra ID tenant identifier | `12345678-1234-1234-1234-123456789012` |
| **Application (client) ID** | Your app’s unique identifier | `87654321-4321-4321-4321-210987654321` |

![Azure App Registration overview page showing IDs](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/azure-app-overview-ids.png) *Figure 6: Locating Tenant ID and Client ID on the Overview page*

Keep these IDs handy – you’ll enter them into the web configurator in the next section.

### Security Considerations

**Q: Is it safe to use the same App Registration for all Ubuntu devices?**

**A:** Yes. The App Registration identifies the *application* (AuthD), not individual devices. All your Ubuntu workstations can share the same Tenant ID and Client ID. User identity is established through the OAuth login flow.

**Q: What if the Client ID is compromised?**

**A:** Since this is a **public client** application (no client secret), the Client ID is not considered sensitive. However, you should still:

* Restrict permissions to minimum required (User.Read only)
...
![Ubuntu Entra ID Configurator web interface](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/configurator-web-interface.png) *Figure 7: Web-based configurator with dark mode enabled*
...
![Configurator with GUID validation errors](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/configurator-validation-errors.png) *Figure 8: GUID validation prevents configuration errors*
...
![Generated configuration with download buttons](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/configurator-generated-output.png) *Figure 9: Generated configuration ready for download*
...
![Rufus creating Ubuntu installation USB](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/rufus-ubuntu-usb-creation.png) *Figure 10: Rufus configuration for Ubuntu 24.04 LTS installation media*
...
![Ubuntu autoinstall progress screen](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/ubuntu-autoinstall-progress.png) *Figure 11: Automated installation in progress (no user input required)*
...
![Ubuntu login screen with Entra ID option](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/ubuntu-login-authd-option.png) *Figure 12: Ubuntu login screen showing “Sign in with authd-msentra” option*
...
![Microsoft device code authentication screen](https://file+.vscode-resource.vscode-cdn.net/c%3A/Users/jensdufour/Git/PUB-Scripts/04_Community/04_EntraID_Ubuntu_Automation/images/microsoft-device-code-flow.png) *Figure 13: Device code authentication flow on mobile device*
...
3. **Enroll devices in Intune** (see our [Ubuntu Intune guide](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-intune-enrollment))
...
See our [Ubuntu Intune Enrollment guide](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-intune-enrollment) for details.
...
4. **For ongoing management**, see our [Ubuntu Intune Enrollment guide](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-intune-enrollment)
...
1. **[Ubuntu Entra ID Authentication with AuthD](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-entra-id-authd)** – Manual configuration guide
2. **[Enrolling Ubuntu in Microsoft Intune](https://file+.vscode-resource.vscode-cdn.net/articles/ubuntu-intune-enrollment)** – Device management
